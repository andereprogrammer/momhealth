...
import {joinSession} from '../../../../api/sessionBooking';
...
>;
interface Props
  extends NativeStackScreenProps<RootStackParamList, 'WelcomeScreen'> {}
const Welcome: React.FC<Props> = ({route}) => {
  // hooks
  ...
  const {id, patientDetails} = useDataProvider();
  const [config, setConfig] = useState<HMSConfig>();
  const [nameDisabled, setNameDisabled] = useState<boolean>(true);
  const [peerName, setPeerName] = useState<string>(patientDetails.patient.name);
  ...
  const onPreviewSuccess = (
    ...
    console.log('Romm success KALSLAS', data.room, previewTracks);
  ...
  const handleJoin = (data: {room: HMSRoom}) => {
    console.log('Romm success FRWEWE', data.room);
  ...
  const onJoinSuccess = (data: {room: HMSRoom}) => {
    console.log('ALSKAKSKAKSKASKAKS', data);
    handleJoin(data);
  };
  ...
  const onStartSuccess = async (
    ...
    try {
      ...
      const token = await joinSession(id)
        .then(res => {
          console.log('jion', res.data.join_room.data.token);
          return res.data.join_room.data.token;
        })
        .catch(e => {
  ...
  const handlePreviewLeave = useCallback(async () => {
    ...
    if (!hmsInstance) {
      return navigate('SessionHomeScreen');
    }
  ...
  return modalType === ModalTypes.PREVIEW && previewTracks ? (
    ...
    <KeyboardAvoidingView
      ...
      <ScrollView
        ...
        <Image style={styles.image} resizeMode="stretch" source={Placeholder} />
        <View>
          <Text style={styles.heading}>Session Starts in Some Time</Text>
